...
/* USER CODE BEGIN PV */
//Vbles para el debounce
#define TEC_INACTIVA 5
#define	TEC_PRESS 6
#define	TEC_LIBRE 7
uint8_t estado_boton [3]= {TEC_INACTIVA, TEC_INACTIVA, TEC_INACTIVA};
uint8_t contador = 0;
uint8_t valor_swich_anterior [3];
uint8_t valor_swich [3];
uint8_t DEBOUNCE_INTERVAL = 10;
//mensaje a transmitir
uint8_t msj1[]="Tec1  libre \n\r";
uint8_t msj2[]="Tec1  pulsada \n\r";
uint8_t msj3[]="Tec2  libre \n\r";
uint8_t msj4[]="Tec2  pulsada \n\r";
	...
	case TEC_PRESS:
		...
		if (valor_swich[i] == valor_swich_anterior[i]|| HAL_GetTick() - contador > DEBOUNCE_INTERVAL){
			...
			if(HAL_GPIO_ReadPin(GPIOA, Tec1_Pin)==1){
				...
				HAL_UART_Transmit(&huart1, msj4, strlen((char*)msj4), 1000);
			}
			contador = 0;
			valor_swich_anterior[i] = 0;
			estado_boton[i] = TEC_LIBRE;
		}else{
			estado_boton[i] = TEC_INACTIVA;
		}
		break;
	case TEC_LIBRE:
		valor_swich[i] = HAL_GPIO_ReadPin(GPIOA, pin_tecla);
		if (valor_swich[i] == 0){
			...
			}if(HAL_GPIO_ReadPin(GPIOA, Tec2_Pin)==0){
				HAL_UART_Transmit(&huart1, msj3, strlen((char*)msj3), 1000);
			}
			estado_boton[i] = TEC_INACTIVA;
		}
		break;
...
int main(void)
{
  ...
  MX_USART1_UART_Init();
  /* USER CODE BEGIN 2 */
  //HAL_UART_Transmit(&huart1, msj, sizeof(msj), HAL_MAX_DELAY);
  /* USER CODE END 2 */
  ...
  while (1)
  {
	  debounce(GPIO_PIN_3,Led1_Pin,1);
	  debounce(GPIO_PIN_4,Led2_Pin,2);
